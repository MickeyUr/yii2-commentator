 ...
 */
class CommentSettings extends \yii\db\ActiveRecord
{
    const YES = 1;
    const NO = 0;
    ...
    {
        return array(
			[['fromEmail', 'adminEmail'], 'required'],
			[['margin', 'levels', 'edit_time', 'max_length_author', 'max_length_content', 'likes_control', 'manage_page_size', 'premoderate', 'notify_admin'], 'integer'],
			[['date_format', 'fromEmail', 'adminEmail'], 'string', 'length'=> [4, 24]],
			[['id', 'date_format', 'margin', 'levels', 'edit_time', 'max_length_author', 'max_length_content', 'likes_control', 'manage_page_size', 'premoderate', 'notify_admin', 'fromEmail', 'adminEmail'], 'safe', 'on'=>'search'],
        );
    }
